<?php
    ...
    if(!empty($_GET['idTreatment'])) {
        ...
        echo $treatmentEndReal = date('Y-m-d');
        ...
        $refreshStatusRequestQuery = "UPDATE treatments SET statusTreatment = 2 WHERE idTreatment = $idTreatment"; 

        $refreshStatusRequest = $connectionDataBase->prepare($refreshStatusRequestQuery);
        $refreshStatusRequest->execute();

        $refreshStatusRequestQuery = "UPDATE treatments SET treatmentEndReal = $treatmentEndReal WHERE idTreatment = $idTreatment"; 
        ...
        $refreshStatusRequestPermissionQuery = "UPDATE treatmentpermissionsrequest SET statusRequest = 2 WHERE idTreatment = $idTreatment"; 
        ...
        $refreshStatusComplementQuery = "UPDATE treatmentpermissionsrequest SET statusComplement = 'Encerrado' WHERE idTreatment = $idTreatment"; 

        $refreshStatusComplement = $connectionDataBase->prepare($refreshStatusComplementQuery);
        $refreshStatusComplement->execute();


        header('Location: ../../../../view/professional/listTreatments/listTreatments.php');
